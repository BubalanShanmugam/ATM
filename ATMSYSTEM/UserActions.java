...
import ATM.bubalanatms.ATMSYSTEM.Notes.Note;
    ...
    public class UserActions {
        ...
        public static Account userLogin(Scanner sc) throws CloneNotSupportedException {//method to the user login
                ...
                for (Account user : ATM.getListofuser()) {//takes the user and admin object element from the userlist.
                    if (user instanceof User) {//checks....user is the User object type.
                        ...
                        while (attempt <= 2) {//to iterate till 3 rd attempt.(pass)
                            ...
                            if (user.getName().contains(username)) {//enters into the body ,if the usernme is equal to the given username.
                                //getting pass
                                System.out.print("Enter Password: ");
                                int password = Integer.parseInt(sc.nextLine());

                                if (user.getName().equals(username) && user.getPin() == password) {//enters into the body ,if the usernme and pass is equal to the given username and pass.
                                    System.out.println("Login successful!");
                                    return user;//if successfull login returns the current user object
                                }
                                else if (user.getName().equals(username) && user.getPin() != password) {//if name correct and pass is wrong
        ...
        public void deposit(double amount, Scanner sc,User user) {//user deposit method
            ...
            if(amount==total){//if total amount and deposited amount is equal
                ...
                user.getTransactions().add(new Transfer(user.getName(), "Deposit", amount));//adding the transaction of the deposited process,to the transactions using the user object
            }
        ...
        public static double performwithdraw(double amount, Note notes, ArrayList<String> statement) {// method for denominateed withdraw
        ...
        public void withdraw(double amount,User user) throws CloneNotSupportedException {//method for the user withdraw
            if (user.getBalance() >= amount) {//enters only if the withdraw amount is greater than the balance
                ...
                ArrayList<String> statement = new ArrayList<String>();//string arrarlist name statement (for denomination statements)
                ArrayList<Note> duplicatenotes = new ArrayList<Note>();//to copy the orginal notesarrylist

                for (Note notes : ATM.getNotesArrayList()) {//from the notesarraylist......gets all the  Notes objects
                    duplicatenotes.add(notes.clone());//adds the orginal notes objects into duplicate list using clone(shallow copy)
                }

                while (amount != 0) {//untill amount not 0
                    for (Note notesinduplicate : duplicatenotes) {//from the duplicatenotes......gets all the  Notes objects
                    ...
                    if (amount == 0) {//if amount gets 0
                        ...
                        user.getTransactions().add(new Transfer(user.getName(), "Withdraw", amount1));//Adding the transaction to the transaction arraylist
